					...
					<form action="" method="post" name="Form" id="Form">
						...
						<div class="project-list">
							<table id="simple-table"
								...
								<thead>
									<th class="center">友情链接ID</th>
									<th class="center">友情链接标题</th>
								...
								<tbody>
									<c:choose>
										<c:when test="${not empty linkslist}">
											<c:forEach items="${linkslist}" var="item" varStatus="vs">
												<tr>
													<td class="center">${item.id}</td>
													<td class="center">${item.link_title}</td>
													<td class="center">${item.link_url}</td>
													<td class="center">${item.link_sort}</td>
													<td class="center"><c:if test="${item.link_status == 1}">
																<span class="label badge-info">显示</span>
															</c:if> <c:if test="${item.link_status == 0}">
																<span class="label badge-danger">隐藏</span>
															</c:if></td>
													<td class="center">
	...
	<script type="text/javascript">
	  ...
	  function dellinks(id){
		if(window.confirm("您确定要删除吗？")){
			Ajax.request("/links/deletelinks",{"data":{"id":id},"success":function(data){
				   if(data.result == "200"){
					   bootbox.alert("删除成功！");
