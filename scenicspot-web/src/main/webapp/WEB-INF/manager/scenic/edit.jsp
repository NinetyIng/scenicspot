...
<body class="gray-bg">
								...
								<div id="tab-1" class="tab-pane active">
									<div class="panel-body">
										...
										<div class="form-group">
											<label class="col-sm-2 control-label"><font
												color="red" size="3px"
												style="font-weight: bold; font-style: italic;">*&nbsp;</font>景区分类：</label>
											<div class="col-sm-2" id="appendCategory">
												<select name="category_id" id="category_id"
													class="form-control">
													<option value="">--请选择景区分类--</option>
													<c:forEach var="category" items="${categorylist}">
														<option data-type='${category.scenic_is_voice}'   value="${category.id}"
															<c:if test="${dataModel.category_id eq category.id}"> selected ='selected'</c:if>>${category.scenic_category}</option>
													</c:forEach>
												</select>
											</div>
										</div>
										...
										<div class="form-group">
											...
											<div class="col-sm-8">
												<input type="text" name="scenic_business_time"
													id="scenic_business_time" class="form-control"
													value="${dataModel.scenic_business_time}">
											</div>
										</div>
										<div class="form-group">
											<label class="col-sm-2 control-label"><font
										...
										<div id="voice_file" class="form-group">
											<label class="col-sm-2 control-label">音频提示：</label>
											<div class="col-sm-8">
												<div class="fileupload fileupload-new"
													data-provides="fileupload">
													<div class="fileupload-new thumbnail"
														style="max-height: 200px;">
														<audio controls="controls" >
														  <source src="${SETTINGPD.IMAGEPATH}${dataModel.scenic_tip_resource}" type="audio/mpeg" />
														</audio>
													</div>
													<div class="fileupload-preview fileupload-exists thumbnail"
														style="max-height: 200px; line-height: 20px;"></div>
													<div>
														<span class="btn btn-default btn-file"> <span
															class="fileupload-new">选择文件</span> <span
															class="fileupload-exists">重选</span> <input type="file"
															name="scenic_tip_resource" id="scenic_tip_resource" accept="audio/mpeg"/>
														</span> <a href="#" class="btn btn-default fileupload-exists"
															data-dismiss="fileupload">移除</a>
													</div>
												</div>
												...
												<div class="form-group">
											<label class="col-sm-2 control-label">播放次序（提示音频和介绍音频播放顺序）：</label>
											<div class="col-sm-8">
												<label class="radio-inline"> <input type="radio"
													<c:if test="${empty dataModel.scenic_tip_order || dataModel.scenic_tip_order eq '1'}">checked='checked'</c:if>
													value="1" name="scenic_tip_order">提示语音在前
												</label> <label class="radio-inline"> <input type="radio"
													value="2"
													<c:if test="${dataModel.scenic_tip_order eq '2'}">checked='checked'</c:if>
													name="scenic_tip_order">提示语音在后
												</label>
											</div>
										</div>
										
										<div class="form-group">
											<label class="col-sm-2 control-label">是否为购票景区：</label>
											<div class="col-sm-8">
												<label class="radio-inline"> <input type="radio"
													<c:if test="${empty dataModel.scenic_is_ticket || dataModel.scenic_is_ticket eq '0'}">checked='checked'</c:if>
													value="0" name="scenic_is_ticket">无需购票
												</label> <label class="radio-inline"> <input type="radio"
													value="1"
													<c:if test="${dataModel.scenic_is_ticket eq '1'}">checked='checked'</c:if>
													name="scenic_is_ticket">需购票
												</label>
											</div>
										</div>
											<div class="form-group need_ticket">
												<label class="col-sm-2 control-label">景区标签：（两标签之间以逗号分割）</label>
												<div class="col-sm-8">
													<input type="text" name="scenic_lable" id="scenic_lable"
														class="form-control" value="${dataModel.scenic_lable}">
												</div>
											</div>
											  <div class="form-group need_ticket">
												<label class="col-sm-2 control-label">门票起售价：</label>
												<div class="col-sm-8">
													<input type="number" name="scenic_ticket_price" id="scenic_ticket_price"
														class="form-control" value="${dataModel.scenic_ticket_price}">
												</div>
											</div>
										<div class="form-group">
    ...
    $(function(){
		...
		$(function(){
			...
			if(!'${dataModel.scenic_is_ticket}'){
				$(".need_ticket").hide();
			}
			$("input[name='scenic_is_ticket']").click(function(){
				if($(this).val() == "1")
					$(".need_ticket").show();
				else
					$(".need_ticket").hide();
		...
		function checkForm() {
			...
			if (!$("#scenic_phone").val().isNotEmpty() || $("#scenic_phone").val().trim().length > 20) {
				alert("联系电话长度为0到20个字符");
				return false;
			}
			if (!$("#scenic_business_time").val().isNotEmpty()) {
				alert("营业时间不能为空");
				return false;
			}
			...
			if("1" == $("#category_id option:selected").attr("data-type") && !'${dataModel.scenic_voice_path}'){
			...
			if ($("#scenic_tip").val().length > 500) {
				bootbox.alert("景区提示应该在500个字符以内");
				return false;
			}
			if (!$("#scenic_click").val()) {
				$("#scenic_click").val(0);
			}
			if ($("#scenic_click").val() == null
					|| $("#scenic_click").val() == ''
					|| !$("#scenic_click").val().isNumber()
					|| $("#scenic_click").val() < 0) {
			...
			if ($("#scenic_radius").val() == null
					|| $("#scenic_radius").val() == ''
					|| !$("#scenic_radius").val().isNumber()
					|| $("#scenic_radius").val() < 0) {
